...
const Spline = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const jsonData = {
      ...
      d: d, // Include the d parameter from the form
    };
  
    console.log(xvectorArray,yvectorArray,d)

    try {
      const response = await axios.post('http://127.0.0.1:5000/interpolation/spline',
        jsonData,
    ...
    <SimpleGrid columns={2} spacing={8}>
      ...
      <Container maxW='550px' color='black' >
        ...
        >
          ...
          <FormControl isInvalid={errors?.d} errortext={errors?.d?.message} isRequired>
            <FormLabel htmlFor="d">Spline Type</FormLabel>
            <Input
              type="text"
              {...register("d")}
              borderColor="#251605"
            ...
            />
            {errors?.d ? (
              <FormErrorMessage>{errors?.d?.message}</FormErrorMessage>
            ) : (
              <FormHelperText></FormHelperText>
            )}
          </FormControl>
