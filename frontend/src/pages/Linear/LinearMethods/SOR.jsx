...
const SOR = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:5000/linear/sor',
        jsonData,
    ...
    <SimpleGrid columns={2} spacing={8}>
      <Container maxW='550px' color='black'>
        ...
        >
          ...
          <FormControl isInvalid={errors?.w} errortext={errors?.w?.message} isRequired>
            <FormLabel htmlFor="w">Spectral Radio (w)</FormLabel>
            <Input
              type="text"
              {...register("w")}
              borderColor="#251605"
            ...
            />
            {errors?.w ? (
              <FormErrorMessage>{errors?.w?.message}</FormErrorMessage>
            ) : (
              <FormHelperText></FormHelperText>
            )}
          </FormControl>
